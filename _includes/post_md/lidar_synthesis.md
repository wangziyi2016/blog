...
1) **Mesh Generation**: Converting real-world environments and objects from sparse point clouds into dense 3D meshes.\\
2) **Scene Composition**: Procedurally composing new scenes by placing dynamic objects into static backgrounds.\\
3) **LiDAR Point Cloud Simulation**: Generating realistic LiDAR point clouds using raycasting physics.
...
- Pick a static background and place dynamic objects (e.g., vehicles, pedestrians, or rare classes) at plausible positions.
- Configure the LiDAR sensor trajectory (e.g., where the ego vehicle is driving)
- Raycast the mesh to generate the LiDAR point cloud.
